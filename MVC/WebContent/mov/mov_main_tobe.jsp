	...
	<div id="sub_content" class="mov_main">
		<div class="container">
			...
			<div class="screen_ranking ranking_box">
				<h2>상영 예정작</h2>
				<%
					int totalCount = toBeMovList.size() / 5 + (toBeMovList.size() % 5 == 0 ? 0 : 1);
				...
				<%
						for (int i = 0; i < 5; i++) {
							int count = 5 * j + i;
							if (toBeMovList.size() > count) {
				%>
					<li>
						<div class="img_box">
							<div class="black_box">
								<a href=MovDetail.mo?movieCd=<%=toBeMovList.get(count).getMovieCd() %>>상세정보</a>
							</div><!-- .black_box -->
							<span class="grade gr_<%=toBeMovList.get(count).getGrade() %>"><%=toBeMovList.get(count).getGrade() %></span>
							<img src="movUpload/<%=toBeMovList.get(count).getPost() %>">							
						</div><!-- .img_box -->
						<div class="mov_info">
							<h3><%=toBeMovList.get(count).getSubjet() %></h3>
							<p>
								<span class="rating">개봉예정일 <b><%=toBeMovList.get(count).getOpenDt() %></b></span>
								<span class="score"><b><%=toBeMovList.get(count).getTotalRating() %></b></span>
							</p>
						</div><!-- .mov_info -->
					</li>
				<%
