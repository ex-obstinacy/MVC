...
public class PreviewListSearchService {

	public int getListCount(String search) {
		
		System.out.println("PreviewListSearchService - getListCount() ");
		int listCount = 0 ;
		
		// 1(공통). Connection 객체 가져오기
		Connection con = getConnection();
		
		// 2(공통). Preview 객체 가져오기
		...
		//    전체 게시물 수 가져오기
		listCount = previewDAO.selectListCount(search);
	...
	public ArrayList<PreviewBean> getArticleList(int page, int limit, String search) {
		...
		//    => 파라미터 : page, limit
		articleList = previewDAO.selectArticleList(page, limit, search);
