...
import svc.GoodsWriteProService;
import vo.ActionForward;
import vo.StoreBean;

public class GoodsWriteProAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("GoodsWriteProAction!"); //여기로 넘어와 지는지 제일 먼저 확인하기
		
		ActionForward forward = null; //만들어 놓고 시작하자!!!
		
		//데이터가 잘 넘어오는지 확인하기!
		// MultiPartRequest 객체를 가져와서
		// 전달받은 파라미터(글쓴이, 비밀번호, 글제목, 글내용, 작성일) 가져오기
...
//		int fileSize = 1048576; // 직접적인 크기를 명시하지 않도록 한다!
		// 작은 단위로 분할해서 해당 크기에 맞게 연산 수행하도록 지정
//		int fileSize = 1024 * 1024; // 1024Byte * 1024Byte = 1,048,576Byte = 1MByte
		...
		// 주의! request 객체의 getParameter() 메스드가 아닌
...
//		String board_file = multi.getFile(arg0);
...
//		String board_file = multi.getOriginalFileName("board_file");
		...
		//전달할 데이터를 StoreBean 객체에 저장
		StoreBean storeBean = new StoreBean();
		storeBean.setCtg(multi.getParameter("goods_ctg"));
		storeBean.setName(multi.getParameter("goods_name"));
		storeBean.setPrice(Integer.parseInt(multi.getParameter("goods_price")));
		storeBean.setSale(Integer.parseInt(multi.getParameter("goods_sale")));
		storeBean.setComponent(multi.getParameter("goods_component"));
		storeBean.setSellCount(Integer.parseInt(multi.getParameter("goods_sellCount")));
		storeBean.setFile(multi.getOriginalFileName("goods_file"));
		storeBean.setContent(multi.getParameter("goods_content"));
		...
		GoodsWriteProService goodsWriteProService = new GoodsWriteProService();
		boolean isWriteSuccess = goodsWriteProService.registArticle(storeBean);
		...
		if (!isWriteSuccess) {
			...
			//	  응답페이지에서 수행할 작업을 기술
			//	  => 모든 작업(자바스크립터, 태그 등)은 문자열로 지정
			out.println("<script>"); //자바스크립트 시작 태그
			out.println("alert('글 등록 실패!')"); //다이얼로그 메세지 출력
			out.println("history.back()"); // 실패시 이전 페이지로 이동
		...
		//2. 포워딩 경로(URL) 지정
		// 주의! 경로명 앞에 / 기호 붙이지 말것!
//		http://localhost:8080/Movie/GoodsList.go
//		"/GoodsList.go" http://localhost:8080/GoodsList.go로 바뀜. 에러!@
