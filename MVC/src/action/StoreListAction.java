...
public class StoreListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// BoardListService 클래스를 통해 게시물 목록 조회 후
				...
				System.out.println("StoreListAction으로 이동!");
				
				ActionForward forward = null;
				
				//페이징 처리를 위한 변수 선언
				int page =1; //현재 페이지 번호 저장할 변수
				int limit =40; // 페이지 당 표시할 게시물 수를 결정하는 변수
				
				//request 객체로 부터 "page" 파라미터가 전달됐을 경우(null이 아닐 경우)
				// 해당 파라미터 값을 page 변수에 저장(String => int 형변환 필요)
				if (request.getParameter("page")!=null) {
					page = Integer.parseInt(request.getParameter("page"));
				}
				
				//GoodsListService 클래스 인스턴스 생성 후
				//getListCount() 메서드를 호출하여 전체 게시물 수(listCount) 가져오기
				StoreListService storeListService = new StoreListService();
				int listCount = storeListService.getListCount();
//				System.out.println("전체 게시물 수 : " + listCount);
				...
				articleList = storeListService.getArticleList(page, limit);
