...
public class EventListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int listCount = eventListService.getListCount();
//		System.out.println("전체 게시물 수 : " + listCount);
		...
		articleList = eventListService.getArticleList(page, limit);
		...
		// 2. 현재 페이지에서 보여줄 시작 페이지 번호(1, 10, 20 페이지 등)
		int startPage = ((int)((double)page / 10 + 0.9) - 1) * 9 + 1;
		
		// 3. 현재 페이지에서 보여줄 마지막 페이지 번호(9, 19, 29 페이지 등)
