...
import svc.NoticeListSearchService;
import svc.PreviewListSearchService;
import vo.ActionForward;

import vo.PageInfo;
import vo.PreviewBean;

public class PreviewListSearchAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("PreviewListSearchAction");
		
		ActionForward forward = null;
		
		// 페이징 처리를 위한 변수 선언
		int page = 1; // 현재 페이지 번호 저장할 변수
		int limit = 9; // 페이지 당 표시할 게시물 수를 결정하는 변수
		...
		// NoticeListService 클래스 인스턴스 생성 후 
		// getListCount() 메서드를 호출하여 전체 게시물 수(listCount) 가져오기
		PreviewListSearchService previewListSearchService = new PreviewListSearchService();
		int listCount = previewListSearchService.getListCount(search);
//		System.out.println("전체 게시물 수 : " + listCount);
		
		// 전체 게시물 목록을 저장할 ArrayList 객체(articleList) 생성
		// => 제네릭 타입으로 BoardBean 타입 지정
		ArrayList<PreviewBean> articleList = new ArrayList<PreviewBean>();
		// BoardListService 클래스의 getArticleList() 메서드를 호출하여
		// 전체 게시물 목록 리턴받아 ArrayList 객체에 저장
		// => 파라미터 : 현재 페이지번호(page), 페이지 당 게시물 수(limit)
		articleList = previewListSearchService.getArticleList(page, limit, search);
		...
		int startPage = ((int)((double)page / 10 + 0.9) - 1) * 9 + 1;
		...
		forward.setPath("/preview/preview_listSearch.jsp");
