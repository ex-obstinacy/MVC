...
import svc.MemberReserveListService;
import vo.ActionForward;
import vo.MovBean;
import vo.PageInfo;
import vo.ReserveBean;

public class MemberReserveListAction implements Action {

	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("MemberReserveListAction");
		...
		MemberReserveListService memberReserveListService = new MemberReserveListService();
		
		// 해당 회원의 예매리스트 갯수 가져오기
		int listCount = memberReserveListService.getReserveListCount(id);
		
		// 전체 예매 목록을 저장할 ArrayList 객체(articleList) 생성
		...
		articleList = memberReserveListService.getReserveList(page, limit, id);
		...
		forward.setPath("/member/member_reserveList.jsp");
