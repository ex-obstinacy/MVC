...
public class PreviewDAO {
	...
	// 글 등록 
	public int insertArticle(PreviewBean previewBean) {
		
		
		System.out.println("PreviewDAO - insertArticle()");
		...
		try {
			...
			String sql ="SELECT MAX(num) FROM preview";
			...
			sql = "INSERT INTO preview(num,subject,content,readcount,date,file,member_id,thumbnail) values(?,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			// NoticeBean 객체로부터 데이터를 꺼내서 쿼리문 ? 대체
			pstmt.setInt(1, num); // 글번호
			pstmt.setString(2, previewBean.getSubject());
			pstmt.setString(3, previewBean.getContent());
			pstmt.setInt(4, previewBean.getReadcount());
			pstmt.setTimestamp(5, previewBean.getDate());
			pstmt.setString(6, previewBean.getFile());
			pstmt.setString(7, previewBean.getMember_id());
			pstmt.setNString(8, previewBean.getThumbnail());
			// -------------------------------------------------------------------
	...
	public int selectListCount() {
		...
		try {
			String sql = "SELECT COUNT(num) FROM preview";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
	...
	// 게시물 목록 조회
	public ArrayList<PreviewBean> selectArticleList(int page, int limit) {
		...
		try {
			...
			articleList = new ArrayList<PreviewBean>();
...
//				System.out.println("제목 : " + article.getSubject());
	...
	// 글 상세 내용 조회
	public PreviewBean selectArticle(int num) {
		// 글번호(num)에 해당하는 레코드를 SELECT
		
		PreviewBean article = null;
		...
		try {
			...
			// 게시물이 존재할 경우 PreviewBean 객체를 생성하여 게시물 내용 저장
			if(rs.next()) {
				article = new PreviewBean();
				article.setNum(rs.getInt("num"));
				article.setSubject(rs.getString("subject"));
				article.setContent(rs.getString("content"));
				article.setReadcount(rs.getInt("readcount"));
	...
	public int updateReadcount(int num) {
		...
		try {
			String sql = "UPDATE preview SET readcount=readcount+1 "
					+ "WHERE num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
...
//			System.out.println("조회수 증가 결과 : " + updateCount);
	...
	// 글수정
	public int updateArticle(PreviewBean article) {
		...
		try {
			String sql = "UPDATE preview SET subject=?,content=?,file=?,thumbnail=? WHERE num=?";
			pstmt =con.prepareStatement(sql);
...
//		System.out.println("글 검색1");
		...
		try {
			...
			String sql = "SELECT COUNT(*) FROM preview where subject like ?";
			pstmt =con.prepareStatement(sql);
	...
	public ArrayList<PreviewBean> selectArticleList(int page, int limit, String search) {
...
//		System.out.println("글 검색2");
		...
		try {
			...
			// 순서번호(board_re_seq) 번호를 기준으로 오름차순 정렬
			// 조회 시작 게시물 번호(startRow)부터 limit 갯수만큼 조회
			String sql = "SELECT * FROM preview where subject like ? "
					+ "ORDER BY num DESC "
					+ "LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1,"%"+search+"%");
