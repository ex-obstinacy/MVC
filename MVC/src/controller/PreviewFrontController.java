...
import action.PreviewListSearchAction;
...
@WebServlet("*.pr") // 서블릿 주소 중 XXX.no 주소에 대한 요청을 전달받아 처리
public class PreviewFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/PreviewWriteForm.pr")) {
			/*
...
//			forward.setRedirect(false); // 기본값이 false 이므로 설정 생략 가능
		...
		}else if(command.equals("/PreviewList.pr")) {
			action = new PreviewListAction();
		...
		}else if(command.equals("/PreviewDetail.pr")) {
			action = new PreviewDetailAction();
		...
		} else if(command.equals("/PreviewModifyForm.pr")) {
			action = new PreviewModifyFormAction();
		...
		} else if(command.equals("/PreviewModifyPro.pr")) {
			action = new PreviewModifyProAction();
		...
		else if(command.equals("/PreviewDeletePro.pr")){
			action = new PreviewDeleteProAction();
		...
		}else if(command.equals("/PreviewListSearch.pr")) {
			action=new PreviewListSearchAction();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
