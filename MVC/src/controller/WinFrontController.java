...
import action.WinDeleteProAction;
import action.WinDetailAction;
import action.WinListAction;
import action.WinListSearchAction;
import action.WinModifyFormAction;
import action.WinModifyProAction;
import action.WinRandomAction;
import action.WinWriteProAction;
import vo.ActionForward;

@WebServlet("*.wi") // 서블릿 주소 중 XXX.wi 주소에 대한 요청을 전달받아 처리
public class WinFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/WinWriteForm.wi")) {
			/*
...
//			forward.setRedirect(false); // 기본값이 false 이므로 설정 생략 가능
		...
		else if(command.equals("/WinList.wi")) {
			action = new WinListAction();
		...
		else if(command.equals("/WinDetail.wi")) {
			action = new WinDetailAction();
			...
			else if(command.equals("/WinModifyForm.wi")) {
			action = new WinModifyFormAction();
		...
		else if(command.equals("/WinModifyPro.wi")) {
			action = new WinModifyProAction();
		...
		else if(command.equals("/WinDeletePro.wi")){
			action = new WinDeleteProAction();
		...
		else if(command.equals("/WinListSearch.wi")) {
			action=new WinListSearchAction();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
	}else if(command.equals("/WinRandom.wi")) {
		action= new WinRandomAction();
		try {
			forward = action.execute(request, response);
		} catch (Exception e) {
			e.printStackTrace();
