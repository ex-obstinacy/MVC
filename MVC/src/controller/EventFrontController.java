...
import action.EventListSearchAction;
...
public class EventFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/EventWriteForm.ev")) {
			/*
...
//			forward.setRedirect(false); // 기본값이 false 이므로 설정 생략 가능
		} else if(command.equals("/EventWritePro.ev")) { // BoardWritePro.bo 서블릿 요청에 대한 처리
		...
		}else if(command.equals("/EventList.ev")) {
		...
		}else if(command.equals("/EventDetail.ev")) {
		...
		} else if(command.equals("/EventModifyForm.ev")) {
		...
		} else if(command.equals("/EventModifyPro.ev")) {
		...
		else if(command.equals("/EventDeletePro.ev")){
